...
In queste competizioni prendono parte diversi team, i quali vengono associati ognuno ad una macchina solitamente linux,
...
Ogni flag viene generata e inserita nel servizio dei vari team ciclicamente ad ogni tick, con una flag sempre differente dalle altre ed ha usualmente una durata massima di valenza (ad esempio 5 tick):
...
al solo scopo di renderli usufruibili per eseguire penetration testing (non tutte le competizioni prevedono la presenza dei \texttt{NOP team}).\\
...
che non sono dei target di interesse nella gara e per cui non è consentita l'esecuzione di attacchi mirati.
...
in cui la rete rimane non accessibile: i team avranno accesso unicamente alle loro macchine, mentre l'accesso ai servizi degli avversari verrà negato.\\
...
I team iniziano a lavorare, tentando di difendersi dai primi attacchi, e a loro volta di attaccare rubando le prime flag agli avversari, tutto ciò mantenendo sempre i loro servizi attivi e funzionanti.
...
\texttt{NOTA:} tutto quello descritto in questo capitolo è possibile simularlo e sperimentarlo tramite un mio progetto chiamato \texttt{Oasis}\footcite{\url{https://github.com/domysh/Oasis}}{oasis_gh},
...
svolgere competizioni A/D di medio-piccole dimensioni in un ambiente realizzato unicamente tramite l'utilizzo di container, con funzionamento e regolamento simile a quello di CyberChallenge\footcite{\url{https://rules.ad.cyberchallenge.it/}}{cyberchallenge_ad_rules}.
...
diverse operazioni necessarie durante la gara. Ogni team può sviluppare e utilizare anche tool che facilitano (o perfino che automatizzino) l'attacco e la difesa dei servizi.\\
In generale sono frequenti nel loro utilizzo le seguenti categorie di tool:

\begin{itemize}
    \setlength{\itemsep}{2pt}
    \setlength{\parskip}{2pt}
    \item \textbf{Traffic Analyzer}: permettono di analizzare il traffico sulla macchina, in modo da riconoscere e visionare il traffico dei checker e degli attaccanti (anche se anonimizzato).
    ...
    Esistono tool scritti appositamente per queste competizioni che permettono di analizzarlo in modo più efficiente e veloce, ma anche di riprodurre velocemente
    ...
    \item \textbf{Attacker \& Submitter}: coordinano gli attacchi, assistono alla scrittura degli exploit, ne monitorano l'esecuzione, schedulandola e parallelizzandola.
    ...
    (normalmente) avviene in modo centralizzato per tutte le flag ottenute dagli attacchi: questo poichè in questo modo risulta più semplice coordinarne la consegna, in base alle regole imposte
    ...
    \item \textbf{Proxy e Firewall}: permettono di filtrare il traffico in modo da riconoscere e bloccare gli attacchi. Questi tool possono essere molto utili
    nella difesa, e permettono di bloccare gli attacchi senza di fatto eseguire una modifica sui servizi stessi, spesso diretta conseguenza di malfunzionamenti per errori eseguiti durante questa fase.
    Questo modo di proteggere i servizi, tuttavia, risulta spesso incompleto e superabile, ma se ben progettato e correttamente utilizzato può essere un'ottima soluzione temporanea, se non
