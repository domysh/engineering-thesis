...
Le competizioni CTF di tipo attacco/difesa sono una tipologia di competizioni CTF in cui i partecipanti devono difendere i propri servizi
...
\begin{itemize}
    ...
    \item Altri si occupano di utilizzare il servizio per memorizzare al suo interno una \texttt{flag} (solitamente una stringa generata in modo casuale),
    con un formato ben definito fornito dagli organizzatori, inserita in modo da non essere normalmente accessibile seguendo i normali workflow del servizio.
...
attività del servizio, per cui il gameserver lo andrà a considerare come totalmente o parzialmente inaccessibile o manomesso (dipendentemnte dalle scelte degli organizzatori).\\
...
Ogni flag viene generata e inserita nel servizio dei vari team ciclicamente ad ogni tick (ogni team avrà una flag differente dagli altri), ed ha usualmente una durata massima di valenza (ad esempio 5 tick):
in questo intervallo di tempo è ancora possibile consegnare la flag al gameserver ottenendo punti.\\
...
Si è assunto implicitamente fino ad ora che consegnare le flag del proprio team non da alcun punteggio, nè tantomeno
...
Cioè è possibile poichè il gameserver ha traccia di tutte le flag, di chi le consegna, e delle macchine in cui sono state originariamente inserite dai checker. 
...
in cui la rete di gara è chiusa: i team hanno accesso unicamente alle loro macchine e nè le macchine stesse nè i componenti del team possono comunicare con le macchine degli avversari.\\
...
Ad ogni tick, il gameserver assegna i nuovi punteggi, e i team possono osservare tramite questi dati l'evoluzione della gara stessa, comprendendo quali possono essere le azioni da compiere per massimizzare la crescita il punteggio.
...
La competizione termina ad un determinato orario di fine, dove la rete di gara viene chiusa, e il gameserver interrompe l'azione dei checker, e non accetta più nuove submission di flag.\\
...
svolgere una competizione A/D di medio-piccole dimensioni in un ambiente realizzato unicamente tramite l'utilizzo di container, con funzionamento e regolamento simile a quello di CyberChallenge.
...
\begin{itemize}
    ...
    Analizzare il traffico diventa cruciale dal momento in cui un nuovo attacco è attivo sulla rete, poichè permette ai team di analizzarlo l'attacco, riconoscere la vulnerabilità
    ...
    le stesse sequenze di attacco, come con ad esempio \texttt{Caronte}\footcite{\url{https://github.com/eciavatta/caronte}}{caronte} o
    ...
    normalmente avviene in modo centralizzato per tutte le flag di tutti gli attacchi: questo poichè risulta spesso più semplice coordinare la consegna delle flag in base alle regole imposte
    dagli organizzatori sul numero di submission, implementate per evitare attacchi di tipo \texttt{bruteforce} o \texttt{DoS} all'infrastruttura.
    ...
    tuttavia ha diversi punti deboli e pecca di diverse funzionalità sull'analisi dell'andamento degli attacchi, motivo per il quale ho sviluppato un nuovo tool chiamato
    ...
    nella difesa, e permettono di bloccare gli attacchi senza di fatto eseguire una modifica sui servizi stessi, spesso causa di malfunzionamenti per errori eseguiti durante la modifica.
    Questo modo di progettere i servizi, tuttavia, è spesso incompleto e bypassabile, ma se ben progettato e correttamente utilizzato può essere un'ottima soluzione temporanea, se non
    una soluzione completa in alcune casistiche per la difesa. Spesso però la soluzione più efficace rimane comunque quella di correggere la vulnerabilità.
