...
Come già esplicitato in precedenza, il regex filtering è un modello di filtraggio che permette sì ottime prestazioni (poiché processato, e costruito con linguaggi
...
la maggior parte dei servizi in ambito CTF girano su protocolli quali HTTP o spesso anche su canali TCP, per esporre interfacce di programmi con interfaccia da terminale,
...
\texttt{Python}\footcite{\url{http://python.org/}}{python}, \texttt{nfproxy} è stato sviluppato proprio al fine di superare i precedenti limiti lasciando una
...
\texttt{Nfproxy} non è propriamente di per sé un proxy, ma un modulo che sfrutta l'utilizzo di
...
cambio di configurazione del servizio originale per applicare i filtri, poiché il traffico viene intercettato in
...
\begin{itemize}
    ...
    \item \textbf{Segnalazione degli errori}: il modulo deve segnalare in maniera chiara e precisa gli errori di parsing e di esecuzione dei filtri, facendo noto 
    al programmatore errori inaspettati rilevati e fornendo i log necessari per la risoluzione.
    \item \textbf{Supporto alla gestione dei filtri in maniera immediata}: deve essere possibile disabilitare totalmente o parzialmente i filtri in maniera immediata e
    senza downtime del servizio.
    \item \textbf{Supporto alla parallelizzazione}: il modulo deve supportare la parallelizzazione dei pacchetti, permettendo di sfruttare al meglio le risorse del sistema
    e garantendo un'ottima scalabilità del firewall.
    \item \textbf{Possibilitò di verifica del filtro tramite simulazione}: il modulo deve permettere di verificare il filtro in maniera simulata, permettendo di testare
...
\begin{itemize}
    \setlength{\itemsep}{2pt}
    \setlength{\parskip}{2pt}
    \item \textbf{Evitare la possibilità di conflitti di dati tra i processi}: Elemento critico nella progettazione per cui si devono evitare quanto possibile i conflitti
    tra i vari processi di modo da evitare corruzione di dati, crash improvvisi e comportamenti inaspettati.
    \item \textbf{Evitare l'utilizzo di elementi bloccanti quali le lock}: i lock spesso ricorrono utili nella gestione di risorse condivise, ma possono portare a problemi
    di prestazioni e di deadlock: pertanto uno degli obiettivi fissati è ridurre il loro utilizzo al minimo indispensabile, strutturando il sistema appositamente per evitare 
    casistiche in cui è necessario il loro utilizzo.
    \item \textbf{Evitare copie in memoria, e avvio di processi sovrabbondanti}: Evitando le copie in memoria dei pacchetti, riduciamo il
...
Il produttore quindi accoderà i pacchetti in queste code, che verranno prelevati dai relativi consumatori per essere elaborati.
...
Una delle problematiche citate precedentemente sulla parallelizzazione erano riguardo una caratteristica del linguaggio python,
il \texttt{GIL}\footcite{Understanding the python gil}{beazley2010understanding} (Global Interpreter Lock), che limita l'utilizzo dei thread in python
...
poiché internamente (come ad esempio per il sistema di reference counting) la loro gestione non è implementata in maniera thread-safe e incorrerebbe a comportamenti inaspettati.\\
Inoltre si sottolinea come python in questa casistica gestisce unicamente operazioni CPU-bound, che vanno ulteriormente ad evidenziare gli svantaggi portati dal GIL.\\
...
(se avessimo ordinato ip e porta non considerandoli a coppie, avremmo un identificativo che potrebbe essere facilmente soggetto a conflitti e perfino soggetto a
...
intercettare del traffico, poiché i pacchetti arriveranno direttamente alle fasi di gestioni dei dati per la loro consegna alla socket a livello applicatvo.\\
...
e sulle problematiche di segmenti non ordinati dei pacchetti, che solleva l'applicativo da un numero importante di problematiche che andrebbero gestiste
...
Oltre al contesto creato all'interno dell'oggetto follower di libtins, viene creato uno stato esternamente per lo stesso flusso, per la memorizzazione
...
alle richieste del progetto, sopratutto data la necessaria compatibilità dei moduli e le funzionalità introdotte nel
...
\begin{itemize}
    ...
    \item \textbf{brotli}\footcite{RFC7932, Brotli Compressed Data Format}{rfc7932}: Il seguente algoritmo di compressione è supportato tramite
    ...
    \item \textbf{zstd}\footcite{RFC8478, Zstandard Compression and the application/zstd Media Type}{rfc8478}: Algoritmo di compressione sviluppato da \texttt{Meta},
...
I filtri python definiti per nfproxy devono essere scritti in un singolo file (per servizio) che può contenere una serie di funzioni chiamate
\texttt{pyfilters} che possono richiedere determinati dati, e possono restituire degli \texttt{statement} che definiscono il comportamento da eseguire elaborato
...
Le funzioni possono restituire uno \texttt{statement} che definisce il comportamento da eseguire, tra i principali \texttt{ACCEPT}, \texttt{REJECT} e \texttt{DROP}.
