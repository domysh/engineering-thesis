...
riceve i pacchetti dal kernel e li distribuisce a multiple code FIFO.\@ I consumatori, associati ciascuno a una coda dedicata, gestiscono in parallelo:
\begin{itemize}
    \setlength{\itemsep}{2pt}
    \setlength{\parskip}{2pt}
...
L'unica sezione critica risiede nell'accesso concorrente alle code, protetto da un sistema di lock ottimizzato basato su
...
L'utilizzo delle funzionalità offerte dal \texttt{PEP 684}\footcite{\url{https://peps.python.org/pep-0684/}}{pep648} non sono tuttavia esenti da limitazioni riguardo l'utilizzo
di moduli non compatibili con la \texttt{multi-phase initialization}\footcite{\url{https://peps.python.org/pep-0489/}}{pep489}, moduli scritti in \texttt{Cython}\footcite{\url{https://cython.org/}}{cython}
(che non supporta la \texttt{multi-phase initialization}) e moduli con componenti non thread-safe. Nel corso dello sviluppo queste limitazioni sono state oggetto
di ulteriori problematiche, affrontate in seguito.\\

Un'alternativa considerata è stata l'utilizzo dell'interprete in \texttt{free-threaded mode} di Python 3.13\footcite{\url{https://docs.python.org/3/whatsnew/3.13.html\#whatsnew313-free-threaded-cpython}}{py13_free_threaded}, attualmente sconsigliato per:
\begin{itemize}
    \setlength{\itemsep}{2pt}
    \setlength{\parskip}{2pt}
...
Per il bilanciamento del carico tra i processi consumatori, finalizzato all'isolamento e alla distribuzione equa dei pacchetti, 
è stato implementato un meccanismo di hashing basato su indirizzi IP e porte di origine/destinazione. Questo approccio garantisce una distribuzione uniforme dei pacchetti tra i consumatori,
evitando sovraccarichi e assicurando un'elaborazione parallela efficiente, e allo stesso tempo garantisce che i pacchetti di una stessa connessione vengano sempre gestiti dallo stesso thread.\\
...
Inoltre lo \texttt{stream\_id} è indipendente dal protocollo internet utilizzato, permettendo di identificare connessioni miste IPv4/IPv6:
questo permette di utilizzare la stessa queue per interfaccie di rete differenti, potenzialmente per nuove feature sulla
selezione del traffico da filtrare.\\

Ogni produttore utilizza questo dato come chiave per individurare i dati relativi a quella specifica connessione, utilizzando una \texttt{std::map}\footcite{\url{https://en.cppreference.com/w/cpp/container/map}}{std_map}
diversa per ogni singolo produttore, così implementabile per merito del meccanismo di smistamento dei pacchetti descritto precedentemente.\\

Il produttore ricevuto il pacchetto pertanto lo ingloba in un oggetto \texttt{PktRequest}, che contiene tutte le informazioni necessarie per
l'elaborazione del pacchetto, e lo inserisce nella coda condivisa del produttore di riferimento.\\
Sarà il produttore che una volta elaborato il pacchetto ed inviato il verdetto al kernel, dealloca questa struttura e passa all'elaborazione del
prossimo pacchetto. Nessuna copia in memoria è pertanto necessaria in questo passaggio.
...
Il follower inoltre permette l'integrazione di contesti di stato gestiti esternamente tramite un sistema di callback, utili a gestire
il contesto python allocato per ogni flusso, gestito pertanto in maniera coerente con i dati del follower stesso.\\
Le callback permettono di gestire le seguenti operazioni:
\begin{itemize}
    \setlength{\itemsep}{2pt}
    \item Inizializzazione contesti
    \item Pulizia risorse
    \item Propagazione eventi critici (per chiusure anomale dei flussi)
\end{itemize}

Ogni processo consumatore ha un suo follower dedicato che analizzerà la porzione dei flussi dell'intero traffico a lui dedicata.
